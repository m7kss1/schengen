...
struct ScaleConfig
{
    ...
    auto RowCount(const TableMetadata & table) const { return static_cast<std::uint64_t>(static_cast<double>(table.rows_at_sf1) * factor); }
};
...
inline std::uint64_t CalculateRowCount(std::uint64_t total_rows, std::int32_t part_num, std::int32_t part_count)
{
...
inline std::uint64_t CalculateStartIndex(std::uint64_t total_rows, std::int32_t part_num, std::int32_t part_count)
{
    ValidatePart(part_num, part_count);
...
inline PartitionRange MakePartitionRange(std::uint64_t total_rows, std::int32_t part_num, std::int32_t part_count)
{
...
MakePartitionPlan(const TableMetadata & table, const ScaleConfig & scale, std::int32_t part_num, std::int32_t part_count)
{
...
inline std::vector<PartitionPlan> BuildPartitionPlans(const TableMetadata & table, const ScaleConfig & scale, std::int32_t part_count)
{
