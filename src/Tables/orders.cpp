...
void OrderRowIterator::Reset(std::uint64_t start_row, std::uint64_t end_row, double scale_factor, const TextPool & text_pool)
{
    next_row_ = start_row;
    end_row_ = end_row;
...
bool OrderRowIterator::Next(OrderRow * out)
{
    if (next_row_ >= end_row_)
    {
        return false;
...
bool OrderRowIterator::Done() const
{
    return next_row_ >= end_row_;
}

std::uint64_t OrderRowIterator::NextRowId() const
{
    return next_row_;
}

RandomBoundedInt OrderRowIterator::CreateOrderDateRandom()
{
    return RandomBoundedInt(kOrderDateSeed, kOrderDateMin, kOrderDateMax);
}

RandomBoundedInt OrderRowIterator::CreateLineCountRandom()
{
    return RandomBoundedInt(kLineCountSeed, kLineCountMin, kLineCountMax);
}

RandomBoundedInt OrderRowIterator::CreateQuantityRandom()
{
    return RandomBoundedInt(kLineQuantitySeed, kQuantityMin, kQuantityMax, kLineCountMax);
}

RandomBoundedInt OrderRowIterator::CreateDiscountRandom()
{
    return RandomBoundedInt(kLineDiscountSeed, kDiscountMin, kDiscountMax, kLineCountMax);
}

RandomBoundedInt OrderRowIterator::CreateTaxRandom()
{
    return RandomBoundedInt(kLineTaxSeed, kTaxMin, kTaxMax, kLineCountMax);
}

RandomBoundedLong OrderRowIterator::CreatePartKeyRandom(double scale_factor)
{
...
RandomBoundedInt OrderRowIterator::CreateShipDateRandom()
{
    return RandomBoundedInt(kLineShipDateSeed, kShipDateMin, kShipDateMax, kLineCountMax);
}

std::int64_t OrderRowIterator::MakeOrderKey(std::int64_t order_index)
{
...
std::int64_t OrderRowIterator::CalculatePartPrice(std::int64_t part_key)
{
...
std::string OrderRowIterator::FormatClerk(std::int32_t clerk_id)
{
    char buffer[32];
...
OrderGenerator::OrderGenerator(arrow::MemoryPool * pool)
    : factory_(pool)
    , columns_(factory_)
{
}

const TableMetadata & OrderGenerator::GetTableMetadata() const
{
    return kOrders;
}

void OrderGenerator::Reset(const GeneratorContext & ctx)
{
    ctx_ = ctx;
    columns_.ClearAll();
...
bool OrderGenerator::NextBatch(std::uint64_t max_rows, TableBatch * out)
{
